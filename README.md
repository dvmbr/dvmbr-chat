...
- Room management — create, rename, and delete rooms
- AI mode — toggle an AI assistant per room; all participants see AI responses in real time
- User profile — edit nickname or delete account from the header
- Unread count badges on rooms, reset on entry
- Optimistic message rendering with error feedback
- First-time onboarding tutorial delivered as sequential system messages
- Cascade deletes — removing a user or room cleans up all related data
...
- `PATCH /api/user`: updates the current user's nickname.
- `DELETE /api/user`: deletes the current user's account and clears the auth cookie. All participants and messages are removed via cascade.
...
- `DELETE /api/rooms/:roomId`: deletes a room and all its participants and messages. Only the room creator can delete it.
...
- `GET /api/rooms/:roomId/messages`: returns all messages for a room. The current user must be a participant.
...
- `PATCH /api/rooms/:roomId/ai-mode`: toggles AI mode for a room. Only the room creator can change this.
- `POST /api/rooms/:roomId/ai`: generates an AI response message and persists it. Supports greeting and farewell triggers in addition to regular messages.
...
- `ai:mode-changed`: emitted by a client when AI mode is toggled; broadcast to all clients in the room so they can update their UI in real time.
...
- `User`: represents a browser-based chat user. Each user has a unique `nickname`, a unique `browserToken`, an `isAiBot` flag that marks the AI system user, and an optional `lastRoomId` used for room re-entry. Deleting a user cascades to all their participants and, by extension, their messages.
- `Room`: represents a chat room. Each room has a unique `name`, a creator, an `isAiMode` flag that enables AI responses, participants, and messages. Deleting a room cascades to all its participants and messages.
...
- `MessageType`: classifies messages as `TEXT`, `IMAGE`, `SYSTEM`, or `AI`. `AI` messages are responses generated by the AI assistant. `SYSTEM` messages are used for server-side notifications.
